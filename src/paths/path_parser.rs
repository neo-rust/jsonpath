...
impl<'a> PathParser<'a> {
		...
		F: ParserTokenHandler<'a>,
	{
		...
		if let Some(parse_node) = self.parser.parse_node.as_ref() {
			self.visit(parse_node, parse_token_handler, &|s| token_reader.read_value(s));
		}
...
impl<'a> ParserImpl<'a> {
	pub fn new(input: &'a str) -> Self {
		ParserImpl { token_reader: TokenReader::new(input), parse_node: None }
	}
	...
	fn op(&mut self, prev: ParserNode) -> Result<ParserNode, TokenError> {
		...
		Ok(ParserNode { token, left: Some(Box::new(prev)), right: Some(Box::new(self.term()?)) })
	}
	...
	fn create_node(&mut self, token: ParseToken) -> ParserNode {
		ParserNode { left: None, right: None, token }
	}
}
...
mod path_parser_tests {
	...
	fn parse_path() {
		...
		assert_eq!(run("$.*"), Ok(vec![ParseToken::Absolute, ParseToken::In, ParseToken::All]));

		assert_eq!(
			run("$..*"),
			Ok(vec![ParseToken::Absolute, ParseToken::Leaves, ParseToken::All])
		);
