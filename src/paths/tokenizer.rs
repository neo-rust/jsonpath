...
impl<'a> Tokenizer<'a> {
	pub fn new(input: &'a str) -> Self {
		// trace!("input: {}", input);
		Tokenizer { input: StrReader::new(input) }
	}
	...
	fn whitespace(&mut self) -> Result<Token, TokenError> {
		let span = self.input.take_while(|c| c.is_whitespace()).map_err(to_token_error)?;
...
impl<'a> TokenReader<'a> {
	pub fn new(input: &'a str) -> Self {
		TokenReader { tokenizer: Tokenizer::new(input), curr_pos: 0, err: None, peeked: None }
	}
