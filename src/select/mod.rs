...
use self::{expr_term::*, value_walker::ValueWalker};
	...
	fn visit_array_eof(&mut self) {
		...
		if self.is_last_before_token_match(ParseToken::Leaves) {
			...
			if let Some(Some(e)) = self.selector_filter.pop_term() {
				let selector_filter_consumed = match &e {
					ExprTerm::Number(n) => {
						self.current =
							self.selector_filter.collect_all_with_num(&self.current, to_f64(n));
						self.selector_filter.pop_term();
						true
					},
					ExprTerm::String(key) => {
						self.current =
							self.selector_filter.collect_all_with_str(&self.current, key);
						self.selector_filter.pop_term();
						true
					},
					_ => {
		...
		if let Some(Some(e)) = self.selector_filter.pop_term() {
			match e {
				ExprTerm::Number(n) => {
					self.current =
						self.selector_filter.collect_next_with_num(&self.current, to_f64(&n));
				},
				ExprTerm::String(key) => {
					self.current =
						self.selector_filter.collect_next_with_str(&self.current, &[key]);
				},
			...
			if let Some(current) = &self.current {
				for v in current {
					if let Value::Array(vec) = v {
						let from =
							if let Some(from) = from { abs_index(*from, vec.len()) } else { 0 };

						let to =
							if let Some(to) = to { abs_index(*to, vec.len()) } else { vec.len() };
...
impl SelectorMut {
	...
	fn compute_paths(&self, mut result: Vec<&Value>) -> Vec<Vec<String>> {
		...
		if let Some(origin) = &self.value {
			...
			_walk(origin, &mut result, &mut tokens, &mut visited, &mut visited_order);
		}
