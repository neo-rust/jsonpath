...
use super::utils;
use crate::{
	paths::{tokens::*, ParserTokenHandler, PathParser, StrRange},
	JsonPathError,
};
	...
	fn visit_array_eof(&mut self) {
		if self.is_last_before_token_match(ParseToken::Array) {
			if let Some(Some(e)) = self.selector_filter.pop_term() {
				if let ExprTerm::String(key) = e {
					self.current =
						self.selector_filter.filter_next_with_str(self.current.take(), key);
					self.tokens.pop();
					return
				}
		...
		if self.is_last_before_token_match(ParseToken::Leaves) {
			...
			if let Some(Some(e)) = self.selector_filter.pop_term() {
				let selector_filter_consumed = match e {
					...
					ExprTerm::String(key) => {
						self.current =
							self.selector_filter.collect_all_with_str(self.current.take(), key);
						self.selector_filter.pop_term();
						true
					},
					_ => {
		...
		if let Some(Some(e)) = self.selector_filter.pop_term() {
			match e {
				...
				ExprTerm::String(key) => {
					self.current =
						self.selector_filter.collect_next_with_str(self.current.take(), &[key]);
				},
		...
		if let Some(t) = self.tokens.pop() {
			if self.selector_filter.is_term_empty() {
				...
				match t {
					ParseToken::Leaves => {
						self.current =
							self.selector_filter.filter_all_with_str(self.current.take(), key);
					},
					ParseToken::In => {
						self.current =
							self.selector_filter.filter_next_with_str(self.current.take(), key);
...
impl<'a> JsonSelectorMut<'a> {
	...
	pub fn new_ref(parser: Rc<PathParser<'a>>) -> Self {
		JsonSelectorMut { value: None, parser: Some(parser) }
	}
	...
	fn compute_paths(&self, mut result: Vec<&Value>) -> Vec<Vec<String>> {
		...
		if let Some(origin) = &self.value {
			...
			Self::walk(origin, &mut result, &mut tokens, &mut visited, &mut visited_order);
		}
